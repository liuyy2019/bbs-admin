...
class ReportInvitationTimeline extends React.Component {
                    ...
                    <Timeline mode="alternate">
                        {
                            TimelineValues.map(item => {
                                if (item.status === '0'){
                                    return <Timeline.Item color="red" label="已取消关注">{item.createTime} - {`${item.reportName} 举报${item.title}`}</Timeline.Item>;
                                }
                                return <Timeline.Item >{item.createTime} - {`${item.reportName} 举报${item.title}`}</Timeline.Item>
                            })
                        }
                    </Timeline>
                </Drawer>
